...
class DotProductAttention(MegatronModule):
    ...
    def forward(
        ...
        # verl passes a 2D padding mask [b, s]; expand it to 4D and combine with
        # the causal mask for local (non-TE) attention, mirroring TE behavior.
        if attention_mask is not None and attention_mask.dim() == 2:
            attention_mask = attention_mask.unsqueeze(1).unsqueeze(2)  # [b, 1, 1, s]
            if attn_mask_type == AttnMaskType.causal and attention_mask.size(2) > 1:
                sq = attention_mask.size(2)
                causal = torch.triu(
                    torch.ones(sq, sq, device=attention_mask.device, dtype=torch.bool), diagonal=1
                )
                attention_mask = attention_mask.expand(-1, -1, sq, -1) | causal.unsqueeze(0).unsqueeze(0)
