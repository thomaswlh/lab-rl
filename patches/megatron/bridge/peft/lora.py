...
class LoRA(PEFT, ModuleMatcher):
    ...
    def transform(self, module: nn.Module, name: Optional[str] = None, prefix: Optional[str] = None) -> nn.Module:
        ...
        adapter_types = tuple(
            t for t in (LinearAdapter, LoRALinear, LoRATopKRouter, TELinearAdapter) if isinstance(t, type)
        )
        ...
        if (ans := self.match(module, name, prefix)) is not None:
            _, full_name = ans
            if (
                isinstance(module, nn.Linear) or (te is not None and module.__class__ == te.Linear)
            ) and not is_modelopt_linear(module):
                ...
                elif te is not None and module.__class__ == te.Linear:
