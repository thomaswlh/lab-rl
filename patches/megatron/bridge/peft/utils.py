...
TECL = tuple(
    _c
    for _c in (TEColumnParallelLinear, TELayerNormColumnParallelLinear, TEColumnParallelGroupedLinear)
    if isinstance(_c, type)
)
TERL = tuple(_c for _c in (TERowParallelLinear, TERowParallelGroupedLinear) if isinstance(_c, type))
...
def get_adapter_attributes_from_linear(
    ...
    if isinstance(m, TopKRouter):
        ...
        HAVE_TE and isinstance(TELinear, type) and isinstance(m, TELinear)
